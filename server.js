...
wss.on('connection', (client) => {
  ...
  if (client.url) {
    url = client.url;
  } else if (client.upgradeReq) {
    url = client.upgradeReq.url;
  }
  if  (!url) {
    console.log('Find no URL:', client);
    return;
  }

  // Ensure that the URL starts with '/rtmp/', and extract the target RTMP URL.
  let match;
  if ( !(match = url.match(/^\/rtmp\/(.*)$/)) ) {
    client.terminate(); // No match, reject the connection.
    return;
  }
